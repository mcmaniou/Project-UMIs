...
approach1_mean <-function(r1, full, quality, full2, quality2){
  ...
  consensus <- calculations_mean(grouping, grouping_q)  
  
  #File 2
  grouping2$UMI <- grouping$UMI
  grouping_q2<-merge(grouping2[,c(3,6)], quality2, by="id")
  consensus2 <- calculations_mean(grouping2, grouping_q2)
  
  result <- data.table(UMI = r1, read1 = consensus, read2 = consensus2)
  return(result)
}
...
fwrite(result, paste(getwd(), "/Output.csv", sep = ""), row.names = FALSE, sep = "\t", quote = FALSE)
