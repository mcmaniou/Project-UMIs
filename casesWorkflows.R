pairedR1 <- function(filepath1, filepath2, outputsFolder, 
  ...
  for(i in test_full$UMI){
    ...
    result_mean[[i]] = groupingFunction(i, full, quality, full2, quality2, UMIlength)
...
pairedR1R2 <- function(filepath1, filepath2, outputsFolder, 
  ...
  reads1 <- reads1[1001:2000]
  reads2 <- reads2[1001:2000]
  #data preparation 
  ...
  test_full <- as.data.table(cbind(full$id, full$UMI,full2$UMI))
  colnames(test_full) <- c("ID","UMI1","UMI2")
  test_full$UMI12 <- paste0(test_full$UMI1,test_full$UMI2)
  ...
  test_full <- group_by(test_full,UMI12) %>%
    summarise(UMI12 ,UMI1, UMI2, IDs = paste(ID,collapse ="|"),)%>%
    unique()
  ...
  for(i in c(1:nrow(test_full))){
    ...
    result_mean[[i]] = groupingPairedR1R2(test_full$IDs[i], full, quality, full2, quality2, test_full$UMI12[i])
  ...
  newUMIs <- UMI12correction(test_full,result_mean,sequenceDistance, UMIdistance)
  ...
  for(i in newUMIs){
    ...
    consensus_mean[[i]] = groupingFinalPairedR1R2(i, full, quality, full2, quality2,result_mean, UMIlength, test_full[,c("UMI12","IDs")])
...
single <- function(filepath1, outputsFolder, 
  ...
  for(i in test_full$UMI){
    ...
    result_mean[[i]] = groupingFunctionSingle(i, full, quality, UMIlength)
