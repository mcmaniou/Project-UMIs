groupingFunction <-function(r1, full, quality, full2, quality2, UMIlength){
...
groupingPairedR1R2 <-function(ids, full, quality, full2, quality2, UMI12){
  
 #reads with specific UMI
  grouping = full[which(full$id %in% ids), ]
  grouping2 = full2[which(full2$id %in% ids), ]
  
  quality = quality[which(quality$id %in% ids), ]
  quality2 = quality2[which(quality2$id %in% ids), ]
...
groupingFunctionSingle <-function(r1, full, quality, UMIlength){
...
UMI12correction <- function(test_full,first_consensus, sequenceDistance, UMIdistance){
  ...
  while((nrow(test_full)>1)){
    ...
    for (i in iterations){
      ...
      base_dist1 <- stringDist(c(test_full$UMI1[1], test_full$UMI1[i]), method = "hamming") 
      base_dist2 <- stringDist(c(test_full$UMI2[1], test_full$UMI2[i]), method = "hamming") 
